...
- **Signal Alignment**: Synchronization of differing sampling rates (PPG at 176 Hz, ACC at 52 Hz) using linear interpolation
...
4. **Model Training**: Run src/main.py to train the ANN model or skip this step and use the already existing models/ann_bp_model.pkl
   - Note: The prototype was only trained on data record 041 and for this reason, step 4 shouldn't be skipped
   - Download as many data records as possible from the MIMIC database to improve generalization, reduce overfitting, and capture complex patterns
...
8. **Blood Pressure Estimation**: Run src/measure_bp.py to predict systolic and diastolic blood pressure from the collected data
9. **Reference Values**: Measure reference blood pressure values using a cuff and add these values within src/bp_evaluation.py (line 41 and 42)
