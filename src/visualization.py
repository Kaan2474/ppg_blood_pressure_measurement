...
def create_scatter_plot(Y_test, Y_pred, mae_sbp, sd_sbp, mae_dbp, sd_dbp):
    ...
    # Plot systolic blood pressure predictions
    plt.subplot(1, 2, 1)
    ...
    # Plot diastolic blood pressure predictions
    plt.subplot(1, 2, 2)
...
def create_histogram(Y_pred, Y_test):
    ...
    # Systolic blood pressure error histogram
    plt.subplot(1, 2, 1)
    plt.hist(sbp_error, bins=30, color='maroon', edgecolor='black', alpha=0.7)
    plt.title("Estimation of error distribution for systolic blood pressure")
    ...
    plt.title("Estimation of error distribution for diastolic blood pressure")
    plt.xlabel("Error in mmHg")
    plt.ylabel("Number of Samples")
    plt.grid(axis='y', alpha=0.5)
...
def visualize_signals(ppg_signals, abp_signals, num_samples=1000):
    ...
    # Top Plot: PPG (input values)
    plt.subplot(2, 1, 1)
    plt.plot(ppg_signals[:num_samples], color='black', label='PPG Signal')
    plt.title("Photoplethysmogram (Input Values)")
    plt.xlabel("Samples")
    ...
    plt.title("Arterial Blood Pressure (Target Values)")
    plt.xlabel("Samples")
